...
def score_to_gradient_color(score: float, vmin: float, vmax: float) -> str:
    """Map a score in [vmin, vmax] to a smooth yellow->orange->red hex color.
    Produces a continuous gradient rather than the four discrete category colors."""
    if vmax <= vmin:
        t = 0.5
    else:
        t = max(0.0, min(1.0, (score - vmin) / (vmax - vmin)))
    # Piecewise interpolation: pale yellow (255,255,204) -> orange (253,141,60)
    # -> dark red (189,0,38), matching a YlOrRd feel.
    stops = [
        (0.0, (255, 255, 204)),
        (0.5, (253, 141, 60)),
        (1.0, (189, 0, 38)),
...
def build_map(g: Graph) -> None:
    ...
    # JS toggle: the low-zoom overview layer below ZOOM_THRESHOLD, per-building
    # markers at or above. Prefer the gradient as the overview layer; fall back to
    # the flat zone heatmap if the grid has not been generated.
    overview_layer = grid_layer if grid_layer is not None else zone_layer
    default_visible = {"HighRisk", "ExtremeRisk"}
    inject_zoom_toggle_js(m, overview_layer, list(layers.values()), default_visible)
    ...
    <div style="position:fixed;bottom:30px;left:30px;z-index:9999;
      ...
      <br>
      <small><b>Zoom &lt; {ZOOM_THRESHOLD}:</b> 100 m sub-zone risk gradient.<br>
      <b>Zoom ≥ {ZOOM_THRESHOLD}:</b> individual building markers.<br>
      Toggle the flat zone heatmap in the layer control.</small><br>
      <br>
    ...
    # Frame the 4-zone study area on load so the square is centered and fills the view.
    m.fit_bounds([[48.77087, 9.17693], [48.78882, 9.20422]])
    m.save(str(MAP_FILE))
